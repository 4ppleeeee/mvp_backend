...
class FakeResponse:
    def __init__(self, *, url: str = "", json_data: dict | None = None, content: bytes = b"", status_error: Exception | None = None) -> None:
        self.url = url
...
class FakeSession:
    ...
    def head(self, url: str, **kwargs) -> FakeResponse:
        ...
        return FakeResponse(url="https://www.douyin.com/video/7351234567890123456", status_error=self.head_status_error)
...
def test_douyin_client_resolves_short_link_and_uses_actual_video_not_background_music() -> None:
    ...
    client = DouyinApiClient(session=session, token_client=FakeTokenClient(["x" * 120]), signer=StaticSigner())
...
def test_douyin_client_uses_final_short_link_url_even_when_head_returns_404() -> None:
    from app.ingestion.douyin_api import DouyinApiClient
...
def test_douyin_client_retries_transient_ms_token_failures_without_storing_token() -> None:
    ...
    client = DouyinApiClient(session=FakeSession(_detail()), token_client=token_client, signer=StaticSigner(), token_attempts=2)
...
def test_douyin_adapter_returns_metadata_and_extracts_audio_from_temporary_video(monkeypatch, tmp_path: Path) -> None:
    ...
    media = DouyinMedia("7351234567890123456", "北京五分钟攻略", "https://media.example/download.mp4", 300.0, "途家", "2024-06-24", "https://cover.example/cover.jpg")
    ...
    assert metadata == MediaMetadata(title="北京五分钟攻略", source_platform="douyin", canonical_url="https://v.douyin.com/9DLOqEFBaMM/", duration_seconds=300.0, author="途家", published_at="2024-06-24", thumbnail_url="https://cover.example/cover.jpg")
    assert Path(audio.path).read_bytes() == b"audio"


def test_default_douyin_adapter_receives_configured_media_egress_policy() -> None:
    from app.ingestion.adapters import default_video_adapters

    adapters = default_video_adapters(MediaEgressPolicy("http://proxy.example:8080"))
    douyin = next(adapter for adapter in adapters if getattr(adapter, "platform", None) == "douyin")

    assert douyin._media_egress_policy.proxy_url == "http://proxy.example:8080"
    assert douyin._api_client._policy.proxy_url == "http://proxy.example:8080"
